...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/budgets/{budget_id}/entries/bulk-import", post(bulk_import))
        .route("/budgets/{budget_id}/entries/recurring", post(create_recurring))
        .route("/budgets/{budget_id}/entries/split", post(create_split))
        .route("/entries", get(list_all_entries).post(create_entry))
        .route("/entries/{entry_id}", get(get_entry).patch(update_entry).delete(delete_entry))
        .route("/entries/{entry_id}/recurrence", patch(update_recurrence).delete(cancel_recurrence))
        .route("/entries/{entry_id}/split-legs", get(list_split_legs))
        .route("/entries/{entry_id}/comments", get(list_comments).post(add_comment))
        .route("/comments/{comment_id}", patch(edit_comment).delete(delete_comment))
        .route("/entries/{entry_id}/attachments", get(list_attachments).post(attach_file))
...
async fn client(state: &AppState) -> ApiResult<EntryServiceClient<Channel>> {
    EntryServiceClient::connect(state.entry_grpc_url.clone())
        .await.map_err(|e| map_status(Status::internal(e.to_string())))
}
...
#[derive(Deserialize)]
struct CommentBody { body: String }
#[derive(Deserialize)]
struct AttachBody { file_id: String, file_name: Option<String> }
...
async fn create_entry(
    ...
    let resp = c.create_entry(with_user(&headers, pb::CreateEntryRequest {
...
async fn get_entry(
    ...
    let mut c = client(&state).await?;
    let resp = c.get_entry(with_user(&headers, pb::GetEntryRequest { entry_id })?)
        .await.map_err(map_status)?.into_inner();
    Ok(Json(map_entry(resp.entry.as_ref().unwrap_or(&pb::Entry::default()))))
}
...
async fn list_entries(
    ...
    let params = pb::ListParams {
        q: q.q, kind: q.kind, category_id: q.category_id,
        date_from: q.date_from, date_to: q.date_to,
        amount_min: q.amount_min, amount_max: q.amount_max,
        tags: q.tags, sort_by: q.sort_by, sort_dir: q.sort_dir,
        page: q.page, page_size: q.page_size,
    };
    let resp = c.list_entries(with_user(&headers, pb::ListEntriesRequest { budget_id: Some(budget_id), params: Some(params), budget_ids: vec![] })?)
        .await.map_err(map_status)?.into_inner();
...
async fn list_all_entries(
    ...
    let mut c = client(&state).await?;
    let budget_ids: Vec<String> = q.budget_ids
        .as_deref()
        .unwrap_or("")
        .split(',')
        .filter(|s| !s.is_empty())
    ...
    let resp = c.list_entries(with_user(&headers, pb::ListEntriesRequest { budget_id: None, params: Some(params), budget_ids })?)
        .await.map_err(map_status)?.into_inner();
    let entries: Vec<serde_json::Value> = resp.entries.iter().map(map_entry).collect();
    let meta = resp.meta.as_ref().map_or(
        serde_json::json!({"page":1,"page_size":20,"total_pages":1,"total_rows":0}),
...
async fn update_entry(
    ...
    let mut c = client(&state).await?;
    let resp = c.update_entry(with_user(&headers, pb::UpdateEntryRequest {
        entry_id,
...
async fn bulk_import(
    ...
    let mut c = client(&state).await?;
    let rows = body.rows.into_iter().map(|r| pb::BulkImportRow {
    ...
    let resp = c.bulk_import_entries(with_user(&headers, pb::BulkImportEntriesRequest { budget_id, rows })?)
        .await.map_err(map_status)?.into_inner();
    ...
    Ok(Json(serde_json::json!({"imported_count": resp.imported_count, "error_count": resp.error_count, "results": results})))
}
...
async fn add_comment(
    ...
    let mut c = client(&state).await?;
    let resp = c.add_comment(with_user(&headers, pb::AddCommentRequest { entry_id, body: body.body })?)
        .await.map_err(map_status)?.into_inner();
...
async fn edit_comment(
    ...
    let mut c = client(&state).await?;
    let resp = c.edit_comment(with_user(&headers, pb::EditCommentRequest { comment_id, body: body.body })?)
        .await.map_err(map_status)?.into_inner();
...
async fn delete_comment(
    ...
    let mut c = client(&state).await?;
    c.delete_comment(with_user(&headers, pb::DeleteCommentRequest { comment_id })?)
        .await.map_err(map_status)?;
...
async fn list_comments(
    ...
    let mut c = client(&state).await?;
    let resp = c.list_comments(with_user(&headers, pb::ListCommentsRequest { entry_id })?)
        .await.map_err(map_status)?.into_inner();
...
async fn attach_file(
    ...
    let mut c = client(&state).await?;
    let resp = c.attach_file(with_user(&headers, pb::AttachFileRequest {
        entry_id, file_id: body.file_id, file_name: body.file_name.unwrap_or_default(),
    })?).await.map_err(map_status)?.into_inner();
...
async fn remove_attachment(
    ...
    let mut c = client(&state).await?;
    c.remove_attachment(with_user(&headers, pb::RemoveAttachmentRequest { attachment_id })?)
        .await.map_err(map_status)?;
...
async fn list_attachments(
    ...
    let mut c = client(&state).await?;
    let resp = c.list_attachments(with_user(&headers, pb::ListAttachmentsRequest { entry_id })?)
        .await.map_err(map_status)?.into_inner();
...
async fn create_recurring(
    ...
    let mut c = client(&state).await?;
    let resp = c.create_recurring_entry(with_user(&headers, pb::CreateRecurringEntryRequest {
        budget_id,
        category_id: body.category_id.unwrap_or_default(),
        kind: body.kind.unwrap_or(1),
        amount: body.amount,
...
async fn update_recurrence(
    ...
    let mut c = client(&state).await?;
    let resp = c.update_recurrence_rule(with_user(&headers, pb::UpdateRecurrenceRuleRequest {
        entry_id,
...
async fn cancel_recurrence(
    ...
    let mut c = client(&state).await?;
    let resp = c.cancel_recurrence(with_user(&headers, pb::CancelRecurrenceRequest { entry_id })?)
        .await.map_err(map_status)?.into_inner();
    Ok(Json(map_entry(&resp)))
}
...
async fn create_split(
    ...
    let mut c = client(&state).await?;
    let legs = body.legs.into_iter().map(|l| pb::SplitLeg {
        entry_id: String::new(),
    ...
    let resp = c.create_split_entry(with_user(&headers, pb::CreateSplitEntryRequest {
        budget_id,
        kind: body.kind.unwrap_or(1),
...
async fn list_split_legs(
    ...
    let mut c = client(&state).await?;
    let resp = c.list_split_legs(with_user(&headers, pb::ListSplitLegsRequest { entry_id })?)
        .await.map_err(map_status)?.into_inner();
    let legs: Vec<serde_json::Value> = resp.legs.iter().map(map_entry).collect();
    Ok(Json(serde_json::json!({"split_group_id": resp.split_group_id, "legs": legs})))
}
